    ...
    <section className="projects-section pt-20 md:pt-40 md:mx-20 space-y-5 p-7">
      <div className="projects-header space-y-6">
        ...
        <p className="text-[#111827] text-sm md:text-2xl">
         Work that reflects my passion for building scalable, user-focused digital
         products using modern web and decentralized technologies.
        </p>
      </div>
...
const ImageGallery = ({
  ...
  const scroll = (direction: "left" | "right") => {
    if (scrollContainerRef.current) {
      const scrollAmount = direction === "left" ? -370 : 370; // Image width + gap
        ...
        >
          {images.map((img, imgIndex) => (
            <img
              ...
              className="w-[150px] h-[150px] object-contain rounded-lg shrink-0 hover:scale-105 transition-all duration-300 shadow-md bg-white"
            />
          ))}
        </div>
        {canScrollRight && (
        ...
        >
          {images.map((img, imgIndex) => (
            <img
              ...
              src={img}
              alt={`${title} - snapshot ${imgIndex + 1}`}
              className="w-[350px] h-[350px] object-contain rounded-lg shrink-0 hover:scale-105 transition-all duration-300 shadow-md bg-white"
            />
          ))}
        </div>
        {canScrollRight && (
